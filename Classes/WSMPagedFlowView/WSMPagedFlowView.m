...
@implementation WSMPagedFlowView
...
- (void)initialize{
    ...
    subviewClassName = @"WSMPagedFlowCell";
...
- (void)autoNextPage {
    ...
    switch (self.orientation) {
        case WSMPagedFlowViewOrientationHorizontal:{
            
            [_scrollView setContentOffset:CGPointMake(self.page * _pageSize.width, 0) animated:YES];
            break;
        }
        case WSMPagedFlowViewOrientationVertical:{
...
- (void)queueReusableCell:(WSMPagedFlowCell *)cell{
    [_reusableCells addObject:cell];
}

- (void)removeCellAtIndex:(NSInteger)index{
    WSMPagedFlowCell *cell = [_cells objectAtIndex:index];
    ...
    switch (self.orientation) {
        case WSMPagedFlowViewOrientationHorizontal:{
            ...
            for (NSInteger i = self.visibleRange.location; i < self.visibleRange.location + _visibleRange.length; i++) {
                WSMPagedFlowCell *cell = [_cells objectAtIndex:i];
...
- (void)setPageAtIndex:(NSInteger)pageIndex{
    ...
    WSMPagedFlowCell *cell = [_cells objectAtIndex:pageIndex];
    ...
    if ((NSObject *)cell == [NSNull null]) {
        ...
        cell.didSelectCellBlock = ^(NSInteger tag, WSMPagedFlowCell *cell) {
...
////////////////////////////////////////////////////////////////////////////////////////////////////
#pragma mark -
#pragma mark WSMPagedFlowView API
...
- (void)reloadData {
    ...
    for (UIView *view in self.scrollView.subviews) {
        if ([NSStringFromClass(view.class) isEqualToString:subviewClassName] || [view isKindOfClass:[WSMPagedFlowCell class]]) {
    ...
    if (_needsReload) {
        ...
        switch (self.orientation) {
            case WSMPagedFlowViewOrientationHorizontal://横向
...
- (WSMPagedFlowCell *)dequeueReusableCell{
    WSMPagedFlowCell *cell = [_reusableCells lastObject];
    if (cell)
    {
...
//点击了cell
- (void)singleCellTapAction:(NSInteger)selectTag withCell:(WSMPagedFlowCell *)cell {
